...
class Chat():
    ...
    def suggest_new_questions(self):
        with open("backend/prompt/explain_more.txt", "r") as f:
            explain_more_prompt = [HumanMessage(content=f.read().format(messages_content=[message.content for message in self.messages]))]
        explain_more_questions = self.chatgpt(explain_more_prompt).content
        
        with open("backend/prompt/in_depth.txt", "r") as f:
            in_depth_prompt = [HumanMessage(content=f.read().format(messages_content=[message.content for message in self.messages]))]
        in_depth_questions = self.chatgpt(in_depth_prompt).content

        with open("backend/prompt/in_width.txt", "r") as f:
            in_width_prompt = [HumanMessage(content=f.read().format(messages_content=[message.content for message in self.messages]))]
